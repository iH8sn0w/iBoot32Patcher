...
int patch_boot_args(struct iboot_img* iboot_in, const char* boot_args) {
	...
	if(strlen(boot_args) > strlen(DEFAULT_BOOTARGS_STR)) {
		...
		*(uint32_t*)default_boot_args_xref = (uint32_t) GET_IBOOT_ADDR(iboot_in, reliance_cert_str_loc);
	...
	if(!_ldr_rd_boot_args) {
		uint32_t default_boot_args_str_loc_with_base = (uint32_t) GET_IBOOT_FILE_OFFSET(iboot_in, default_boot_args_str_loc) + get_iboot_base_address(iboot_in->buf);
...
int patch_cmd_handler(struct iboot_img* iboot_in, const char* cmd_str, uint32_t ptr) {
	...
	if(!cmd) {
		printf("%s: Unable to find a ref to \"%s\".\n", __FUNCTION__, cmd_ptr_str_loc);
		return 0;
	}
	
	printf("%s: Found the cmd string reference at %p\n", __FUNCTION__, GET_IBOOT_FILE_OFFSET(iboot_in, cmd));
